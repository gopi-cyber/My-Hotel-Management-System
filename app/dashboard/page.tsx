    ...
    <main className="flex min-h-screen bg-slate-50 text-slate-800 overflow-hidden font-sans">
      ...
      <aside className="z-20 w-72 glass-morphism border-r border-white/60 flex flex-col p-8 space-y-10 h-screen">
          ...
          <nav className="flex-1 space-y-2">
              ...
              <button 
                  onClick={() => alert('Discovery Engine arriving in v2.2')}
                  className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-sm font-semibold text-slate-400 hover:text-indigo-600 hover:bg-white/50 transition-all group"
              >
                  <Compass size={20} className="text-slate-300 group-hover:text-indigo-400" /> Discover
              </button>
              <button 
                  onClick={() => alert('Stay History arriving in v2.2')}
                  className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-sm font-semibold text-slate-400 hover:text-indigo-600 hover:bg-white/50 transition-all group"
              >
                  <Bookmark size={20} className="text-slate-300 group-hover:text-indigo-400" /> My Stays
              </button>
              <button 
                  onClick={() => alert('Full Ledger arriving in v2.2')}
                  className="w-full flex items-center gap-4 px-4 py-3.5 rounded-2xl text-sm font-semibold text-slate-400 hover:text-indigo-600 hover:bg-white/50 transition-all group"
              >
                  <Clock size={20} className="text-slate-300 group-hover:text-indigo-400" /> Ledger
              </button>
          </nav>
